...
module.exports = async req => {
  ...
  const table = fs.readFileSync("./data/current_plan.json", "utf8");
  ...
  console.log("adjust: running completion");
  ...
  console.log("adjust: completion completed");

  fs.writeFileSync("./data/current_plan.json", completion.choices[0].message.content);

  let j;
  try {
    j = JSON.parse(fs.readFileSync("./data/adjustments.json"));
  } catch {
    j = [];
  }

  j.push(time + ": " + feedback);
  fs.writeFileSync("./data/adjustments.json", JSON.stringify(j));
};
